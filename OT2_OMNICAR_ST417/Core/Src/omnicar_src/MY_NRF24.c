 ...
 *
 *  Modified on: 		Feb 20, 2021
 *  Author: 			TEAM OMNICAR
 *  Objective:			Implement this program for our second years project "OMNICAR"
 */

//*** LIST OF HEADER FILES ***//
#include "omnicar_inc/MY_NRF24.h"
#include <string.h>

//*** VARIABLES DECLARATION ***//
#define MAX(x, y) (((x) > (y)) ? (x) : (y))					// macro to check the maximum between tow number
#define MIN(x, y) (((x) < (y)) ? (x) : (y))					// macro to check the minimum between tow number
#define _BOOL(x) (((x)>0) ? 1:0)							// macro to check the state of a bit
...
//CE and CSN pins
static GPIO_TypeDef	 *nrf24_PORT;							// create static variable to add nRF24 PIN
static uint16_t		 nrf24_CSN_PIN;							// create static variable to copy CSN PIN
static uint16_t		 nrf24_CE_PIN;							// create static variable to copy CE PIN

//SPI AND DEBUGGING UART HANDLE
...
void printCONFIGReg(void) {
	...
	sprintf(uartTxBuf,
			"CONFIG reg:\r\n  MASK_RX_DR:  %d\r\n  MASK_TX_DS:  %d\r\n  MASK_MAX_RT:  %d\r\n  EN_CRC:  %d\r\n  CRCO:  %d\r\n  PWR_UP:  %d\r\n  PRIM_RX:  %d\r\n",
			_BOOL(reg_Val & (1 << 6)), _BOOL(reg_Val & (1 << 5)),
			_BOOL(reg_Val & (1 << 4)), _BOOL(reg_Val & (1 << 3)),
			_BOOL(reg_Val & (1 << 2)), _BOOL(reg_Val & (1 << 1)),
			_BOOL(reg_Val & (1 << 0)));
	HAL_UART_Transmit(&nrf24_huart, (uint8_t*) uartTxBuf, strlen(uartTxBuf),
			10);
